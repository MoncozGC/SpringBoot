...
import drug.erp.utils.StringUtil;
...
//        System.out.println(Thread.currentThread()+" 查询离线消息 "+ identityName);
    ...
    public void run() {
        String message = "";
        Log4j.info(">>> 进入Kafka方法: ");
        Properties properties = new Properties();
        Log4j.info(IMPToKafkaProperties.getKafkaHost());
        properties.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, IMPToKafkaProperties.getKafkaHost());
        properties.put(ConsumerConfig.GROUP_ID_CONFIG, IMPToKafkaProperties.getGroupId());
        properties.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, IMPToKafkaProperties.getAutoOffsetReset());
        properties.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, IMPToKafkaProperties.getKeyDecoder());
        properties.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, IMPToKafkaProperties.getValueDecoder());

        try {
            KafkaConsumer<String, String> consumer = new KafkaConsumer<>(properties);
            consumer.subscribe(IMPToKafkaProperties.getTopic());
            while (isRunning) {
                message = null;
    ...
    public void IMPRun(String message) {
        Log4j.info(">>> IM服务启动");
        ...
        while (isRunning){
            try {
                ...
                Log4j.info(">>> IM方法: ");
                Log4j.info(successMessageIdentityQueue.hashCode() + ">>> successMessageIdentityQueue size " + successMessageIdentityQueue.size());
                // 将consumer的消息发送到run中
//                successMessageIdentityQueue.offer(new String[]{message});
                ...
                params.add(param);
                Log4j.info("加入: " + Arrays.toString(param));
            } catch (Exception e) {
                Log4j.error("",e);
            }
            index++;
...
//        properties.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, kafkaConsumerToJava2Properties.getAutoCommitIntervalMs());
